...
const seasonalOrTimeless: QuestionDef = {
  key: 'seasonal',
...
function getSeasonalWeights(currentSeason: Season): Record<Season, number> {
  const weights = {
  ...
  return weights[currentSeason] as Record<Season, number>;
}
...
function getTimelessScore(drink: DecideDrink): number {
  let score = 0;
...
function getSeasonalScore(drink: DecideDrink): number {
  let score = 0;
  ...
  for (const season of seasonalTags) {
    if (drink.tags.includes(season)) {
      score += seasonWeights[season as Season];
    }
  }
...
function detectSouthernHemisphere(): boolean {
